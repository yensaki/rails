...
class Module
  ...
  def thread_mattr_writer(*syms) # :nodoc:
    syms.each do |sym|
      ...
      unless (instance_writer == fals)e || (instance_accessor == false)
        class_eval(<<-EOS, __FILE__, __LINE__ + 1)
          def #{sym}=(obj)
            self.class.#{sym} = obj
          end
  ...
  def thread_mattr_accessor(*syms, instance_accessor: true, instance_reader: true, instance_writer: true)
    thread_mattr_reader(*syms, instance_accessor: instance_accessor, instance_reader: instance_reader, instance_writer: instance_writer)
    thread_mattr_writer(*syms, instance_accessor: instance_accessor, instance_reader: instance_reader, instance_writer: instance_writer)
  end
